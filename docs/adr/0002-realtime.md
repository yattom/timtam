...
## 決定 / Decision
- 1つの会議に対応するコンテナを継続的に起動し、メモリ上で処理することで高速化する。コードも設計もシンプルになる。
- 他のWeb会議システム (ZoomやGoogle Meetなど) に接続する際にもこの形式なら対応範囲が広がる。
...
- オーケストレーション: 常駐ワーカー（ECS Fargate）で状態管理と即時処理。基本的に1会議に1ワーカーを対応させる。
...
- App Runner: WebアプリのSaaSだがストリームを受け取るには不向き。
