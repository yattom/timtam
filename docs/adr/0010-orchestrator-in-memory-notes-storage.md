...
### 5. ECSアーキテクチャとの整合性
- **単一タスク運用**: 現在のオーケストレーターは `desiredCount=1` で稼働中
- **水平スケーリング不要**: 会議の並列処理は単一プロセスで十分に対応可能
  - Bedrock APIのスループット制限がボトルネックであり、プロセス数増は効果薄
- **ステートレスではない前提**: オーケストレーターは元々ステートフルなワーカー設計
...
### ネガティブ
- ❌ タスク再起動時にメモが消失（会議中断のリスク）
- ❌ 複数タスクへのスケールアウトが困難（現時点では不要）
...
3. **ノード実行レイテンシが10秒を超える事例が頻発**（水平スケーリング必要性）
