...
- [ ] worker.tsに2つ目のLLM呼び出しを追加（ハードコード、異なるプロンプト）
- [ ] 2つのLLMが並列実行されるように実装
- [ ] 各LLMの結果をチャットに表示
- [ ] App.tsxでLLM呼び出しログを時系列で表示するUI改善
- [ ] どのLLMがいつ呼ばれたか区別できるようにする（ノードID表示）
- [ ] TemporalNotesStore実装（インメモリKVストア）
- [ ] 1つ目のLLMがメモに書き込む処理を追加
- [ ] 2つ目のLLMがメモを読んでプロンプトに埋め込む処理を追加
