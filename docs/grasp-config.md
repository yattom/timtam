...
WindowBuffer から取得した入力テキスト（発話履歴とチャット履歴）。各行は `[時刻] 発話内容` または `[時刻] [chat] 発言者: メッセージ` の形式。

デフォルトでは音声発話とチャットメッセージを時系列順に混在させて取得します（Timtam自身のチャットは除外されます）。
...
修飾子なしの `{{INPUT}}` はすべての発話・チャットを取得します。

#### INPUT 修飾子

修飾子は `{{INPUT:量:種別}}` の形式で、量と種別を指定できます。種別を省略すると `mix`（音声＋チャット混在）になります。
...
```yaml
{{INPUT:latest5:voice}}   # 最新5件の音声発話のみ
{{INPUT:past30m:chat}}    # 過去30分のチャットメッセージのみ
{{INPUT:all:mix}}         # すべてを混在（明示的に指定）
{{INPUT:latest10:mix}}    # 最新10件を混在
```
...
**デフォルト動作:**
- 修飾子なしの `{{INPUT}}` はすべてを mix で取得
- 種別を省略すると `mix`
- Timtam 自身が送ったチャットメッセージは常に除外

**使用例:**

```yaml
# 例: 最新5行だけを見る（音声＋チャット混在）
promptTemplate: |
  直近の発話を確認してください。
  ---
  {{INPUT:latest5}}
...
**音声発話（voice）:**
```
[14:30:45] [田中太郎] こんにちは、今日のミーティングを始めます
[14:31:02] [鈴木花子] よろしくお願いします
```

**チャットメッセージ（chat）:**
```
[14:31:10] [chat] [山田次郎] 資料を共有します
[14:31:20] [chat] [田中太郎] ありがとうございます
```
...
[14:31:10] [chat] [山田次郎] 資料を共有します
[14:31:20] [chat] [田中太郎] ありがとうございます
[14:31:30] [田中太郎] では始めましょう
```

各行に時刻が付与されるため、時系列を考慮した判断が可能です。

**注意**: Timtam 自身が送ったチャットメッセージの除外は未実装です（#195）。Recall.ai の実際のペイロードを確認したうえで実装予定。現時点では、Timtam のチャットも `[chat]` 行に含まれる可能性があります。
