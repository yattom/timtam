...
// Bootstrap: use VITE_API_BASE_URL to fetch /config first, then switch to server-provided apiBaseUrl
...
let API_BASE: string = (import.meta as any).env?.VITE_API_BASE_URL || '';

function u(path: string) {
  if (!API_BASE) throw new Error('API base URL is not set. Define VITE_API_BASE_URL for bootstrap.');
  return `${API_BASE}${path}`;
}

export async function getConfig(): Promise<PublicConfig> {
  if (!API_BASE) {
    throw new Error('VITE_API_BASE_URL is required to bootstrap /config fetch.');
  }
